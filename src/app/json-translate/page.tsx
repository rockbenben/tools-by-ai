...
import {
  Row, Col, Button, Tooltip, Form, Typography, Input, Select, message, Card, Space, Spin
} from "antd";
...
const JsonTranslate = () => {
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      if (showSimpleInput) {
        // 替换中文逗号为英文逗号，并分割
        const keys = simpleInputKey.replace(/，/g, ',').split(',').filter(k => k.trim() !== '');
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name='keywords'
          content='JsonTranslate, JSON 翻译工具, 多语言翻译, 国际化工具, i18n, 本地化, Google 翻译 API, DeepL 翻译 API, 自动翻译 JSON, 编程语言翻译'
        />
      </Head>
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "24px" }}>
        ...
        >
          JsonTranslate 是一款高效的多语言 JSON 翻译工具，专为开发者和内容创作者设计。支持 Google Translate、DeepL 和 DeepLX 翻译 API，助力快速实现项目的国际化和本地化。无论你是在开发多语言网站、应用程序还是处理多语言数据集，JsonTranslate 都能提供简便的解决方案，轻松将 JSON 文件中的内容翻译为多种目标语言。<br />了解更多：<a href='https://newzone.top/apps/devdocs/json-translate.html'>使用教程</a>；<a href='https://console.cloud.google.com/apis/credentials/key/2c5756a5-5a4c-4d48-993f-e478352dcc64?project=ordinal-nucleus-383814'>
            Google Translate API</a>；<a href='https://www.deepl.com/zh/account/summary'>DeepL API</a>。
        </Paragraph>
        <Row gutter={16}>
          <Col xs={24} lg={12}>
            <Card title='输入区'>
              ...
              <Form.Item>
                <Input.TextArea
                  placeholder='JSON Input，输入要翻译的 JSON'
                  value={jsonInput}
                  onChange={handleJsonInputChange}
                  rows={10}
                />
                {jsonInputError && (
                  <div style={{ color: "red" }}>{jsonInputError}</div>
                )}
              </Form.Item>
            </Card>
          </Col>
